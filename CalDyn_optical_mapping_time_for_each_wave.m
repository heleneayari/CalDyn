...
warning('off','MATLAB:table:ModifiedAndSavedVarnames')
 if strcmp(file(end-2:end),'csv')
    matrix_rough_datat=table2array(readtable(pathname,'Delimiter',','));
 else
matrix_rough_datat=table2array(readtable(pathname));
 end

%% remove unnecessary time columns

col=[1,2:2:size(matrix_rough_datat,2)];
matrix_rough_data=matrix_rough_datat(:,col);
